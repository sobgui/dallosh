...
import { Shield, Database, AlertCircle, Edit, Trash2, Plus } from "lucide-react";
...
export default function PermissionsPage() {
  ...
  const initialTab = (searchParams && validTabs.includes(searchParams.get("tab") || "")) ? searchParams.get("tab")! : "rules";
  ...
  useEffect(() => {
    // Sync tab state with URL on mount (in case of direct navigation)
    if (searchParams) {
  ...
  const checkPermissionsStatus = async () => {
    try {
      ...
      // Check if rules, roles, and permissions collections exist
  ...
  const fetchRuleDropdown = async (search = ruleDropdown.search, page = ruleDropdown.page) => {
    if (!rulesCollection) return;
    setRuleDropdown((prev) => ({ ...prev, loading: true }));
    try {
      const client = getSodularClient();
      let filter: any = {};
      if (search) {
      ...
      const result = await client.ref.from(rulesCollection.uid).query({
        filter,
        sort: { 'data.name': 'asc' },
        take: ruleDropdown.pageSize,
        skip: (page - 1) * ruleDropdown.pageSize
      });
      setRuleDropdown((prev) => ({
        ...prev,
        options: result.data?.list?.map((r: any) => ({ uid: r.uid, name: r.data.name, description: r.data.description })) || [],
        total: result.data?.total || 0,
        loading: false,
      }));
    } catch (e) {
      setRuleDropdown((prev) => ({ ...prev, loading: false }));
    }
  };

  useEffect(() => {
    if (ruleDropdown.open) fetchRuleDropdown();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ruleDropdown.open, rulesCollection]);
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Enable permissions to manage access control rules, roles, and permissions for your database.
            </CardDescription>
          </CardHeader>
        ...
        <TabsContent value="rules" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define conditions and rules for access control
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="roles" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define and manage user roles and role hierarchies
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-end mb-4">
        ...
        <TabsContent value="permissions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define specific permissions and access policies
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-end mb-4">
                <Dialog open={isCreatePermissionDialogOpen} onOpenChange={setIsCreatePermissionDialogOpen}>
                  ...
                  <DialogContent>
                    ...
                    <div className="space-y-4">
                      ...
                      />
                      <Popover open={ruleDropdown.open} onOpenChange={open => setRuleDropdown(prev => ({ ...prev, open }))}>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className="w-full justify-between" type="button">
                            {selectedRule
                              ? selectedRule.name
                              : permissionForm.rule_id
                              ? (ruleDropdown.options.find(o => o.uid === permissionForm.rule_id)?.name || "Select Rule")
                              : "Select Rule"}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="z-[9999] w-full min-w-[var(--radix-dropdown-menu-trigger-width)] p-0" style={{ zIndex: 9999 }}>
                          <Command shouldFilter={false}>
                            <CommandInput
                              placeholder="Search rules..."
                              value={ruleDropdown.search}
                              onValueChange={v => {
                                setRuleDropdown(prev => ({ ...prev, search: v, page: 1 }));
                                fetchRuleDropdown(v, 1);
                              }}
                            />
                            <CommandList>
                              {ruleDropdown.loading && <div className="p-2 text-center text-xs text-muted-foreground">Loading...</div>}
                              <CommandEmpty>No rules found.</CommandEmpty>
                              <CommandGroup>
                                {ruleDropdown.options.map(option => (
                                  <CommandItem
                                    key={option.uid}
                                    value={option.uid}
                                    onSelect={() => {
                                      setPermissionForm(f => ({ ...f, rule_id: option.uid }));
                                      setSelectedRule({ uid: option.uid, name: option.name });
                                      setRuleDropdown(prev => ({ ...prev, open: false }));
                                    }}
                                  >
                            ...
                            <div className="flex items-center justify-between p-2 border-t bg-background">
                              <Button size="sm" variant="outline" disabled={ruleDropdown.page === 1 || ruleDropdown.loading} onClick={() => {
                                const newPage = Math.max(1, ruleDropdown.page - 1);
                                setRuleDropdown(prev => ({ ...prev, page: newPage }));
                                fetchRuleDropdown(ruleDropdown.search, newPage);
                              }}>Prev</Button>
                              <span className="text-xs">Page {ruleDropdown.page} / {Math.ceil(ruleDropdown.total / ruleDropdown.pageSize) || 1}</span>
                              <Button size="sm" variant="outline" disabled={ruleDropdown.page * ruleDropdown.pageSize >= ruleDropdown.total || ruleDropdown.loading} onClick={() => {
                                const newPage = ruleDropdown.page + 1;
                                setRuleDropdown(prev => ({ ...prev, page: newPage }));
                                fetchRuleDropdown(ruleDropdown.search, newPage);
                              }}>Next</Button>
                            </div>
