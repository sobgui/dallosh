...
import { UserCheck, Database, AlertCircle, Edit, Trash2, Plus } from "lucide-react";
...
export default function AuthorizationsPage() {
  ...
  const checkAuthorizationsStatus = async () => {
    try {
      ...
      // Check if authorization junction collections exist
  ...
  useEffect(() => {
    if (usersRolesCollection) loadUsersRoles(usersRolesCollection.uid);
  }, [usersRolesCollection]);

  useEffect(() => {
    if (usersPermissionsCollection) loadUsersPermissions(usersPermissionsCollection.uid);
  }, [usersPermissionsCollection]);

  useEffect(() => {
    if (rolesPermissionsCollection) loadRolesPermissions(rolesPermissionsCollection.uid);
  }, [rolesPermissionsCollection]);
  ...
  const fetchUsersRoles = async () => {
    ...
    try {
      ...
      if (usersRolesSearchInput) {
        // Search users by email/username (match user management)
        ...
        filter = {
          $or: [
            ...
            // fallback to ID search if no matches
  ...
  // Helper fetch functions for dropdowns
  ...
  const fetchRolesOptions = async (search: string) => {
    ...
    const collection = rolesPermissionsCollection || usersRolesCollection;
    const client = getSodularClient();
    let filter: any = {};
    if (search) {
      filter = {
    ...
    // Find the actual roles collection UID
    const rolesCol = await client.tables.get({ filter: { 'data.name': 'roles' } });
  ...
  const fetchPermissionsOptions = async (search: string) => {
    ...
    const collection = rolesPermissionsCollection || usersPermissionsCollection;
    const client = getSodularClient();
    let filter: any = {};
    if (search) {
      filter = {
    ...
    // Find the actual permissions collection UID
    const permissionsCol = await client.tables.get({ filter: { 'data.name': 'permissions' } });
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Enable authorizations to manage user-role and user-permission assignments for ABAC (Attribute-Based Access Control).
            </CardDescription>
          </CardHeader>
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="users-roles" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="users_permissions">Users | Permissions</TabsTrigger>
        ...
        <TabsContent value="users_permissions" className="space-y-4">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="roles-permissions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Assign permissions to roles for role-based access control
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-end mb-4">
